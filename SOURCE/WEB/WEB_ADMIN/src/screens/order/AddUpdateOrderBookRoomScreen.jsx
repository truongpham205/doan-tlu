...
AddUpdateOrderBookRoomScreen.propTypes = {}
...
function AddUpdateOrderBookRoomScreen(props) {
  ...
  const [validateError, setValidateError] = useState(initialValidateError)
  ...
  const getListCustomer = async (search) => {
    try {
      const listCustomerApi = await CustomerApi.getListCustomer({ search })
      setListCustomer(
  ...
  async function onSubmit(data) {
    ...
    dataClone.service_category_id = data.service_category_id.value
  ...
  const addCustomer = async () => {
    ...
    try {
      ...
      setValidateError(initialValidateError)
      notifySuccess(STRING.addSuccess)
    } catch (error) {
      setIsLoadingAddUpdate(false)
      console.log(error)
  ...
  const renderContentLeft = () => {
    return (
      <Col xl="6" className="p-2">
        <div className="px-3 pt-3 box--shadow " style={{ paddingBottom: '2.15rem' }}>
            ...
            <Col sm="9">
              ...
              >
                ...
                {listCustomer.map((customer, index) => (
                  <Option value={customer?.full_name} key={index}>
                    {customer?.full_name}
                  </Option>
                ))}
              </AutoComplete>
    ...
    return (
      <Col xl="6" className="pl-2 py-2 pr-1">
        <div className="px-4 py-3 box--shadow col-12">
          ...
          <FormGroup row>
            <Col sm="3">
              <Label for="exampleEmail">{STRING.departmentName}</Label>
            </Col>
            <Col sm="9">
              <Controller
                name="service_id"
                ...
                placeholder={STRING.departmentName}
                options={listRoom}
                isSearchable={true}
                isClearable={true}
                as={Select}
...
export default withRouter(AddUpdateOrderBookRoomScreen)
